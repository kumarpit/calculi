...
(define (infer e env)
  (let infer ([e e]
    ...
    (match e
      ...
      ;; To support recursion, I need to include the type of fact within the env^ when I am infering
      [`(letrec ([,xs ,es] ...) ,e)
       (begin
         (define env^ (dict-copy env))
         (for ([x xs]
               [e es])
           (match e
             ;; This is the only self-referential construct, so we must ensure its type exist in the env
             ;; when we are type-checking e
             [`(,_ : ,ty) (begin 
                            (dict-set! env^ x (parse-type ty))
                            (infer e env^))]
             [_ (dict-set! env^ x (infer e env^))]))
         (infer e env^))]
...
(check-equal? (run/stlc '(letrec ([x 10]) x)) 10)
(check-equal? (run/stlc '(letrec ([x 1] [y 2]) (+ x y))) 3)
(check-equal?
 (run/stlc
  '(letrec ([fact ((λ (n)
                     (if (zero? n)
...
(check-exn exn:fail?
           (λ () (run/stlc '(letrec ([x ((λ (y) #t) : (-> int int))]) (x 1))))
           "Mismatched types: Expected (IntType) got (BoolType)")
